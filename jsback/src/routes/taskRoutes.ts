...
import { Task, TaskPhoto } from "../entities/task";
import { User } from "../entities/user";
import { Between, In, LessThanOrEqual, Like, MoreThanOrEqual, Not } from "typeorm";
import { Reminder } from "../entities/reminder";

const router = Router();
const taskRepo = AppDataSource.getRepository(Task);
const userRepo = AppDataSource.getRepository(User); 
const photoRepo = AppDataSource.getRepository(TaskPhoto);
    ...
    try {
        ...
        if (searchFilters.phrase) {
            whereConditions.push(
                {
                    title: Like(`%${searchFilters.phrase}%`)
                },
                {
                    description: Like(`%${searchFilters.phrase}%`)
                }
            );
        }
        ...
        if (searchFilters.payment) {
            ...
            if (min !== undefined && max !== undefined) {
                whereConditions.push({ payment: Between(min, max) });
            } else if (min !== undefined) {
                whereConditions.push({ payment: MoreThanOrEqual(min) });
            } else if (max !== undefined) {
                whereConditions.push({ payment: LessThanOrEqual(max) });
            }
        }
        ...
        switch(status) {
            ...
            case 'approved': {
                const users = await userRepo.find({
                    where: {
                        role: Not(In(['customer', 'moderator'])),
                    },
                });
...
router.post("/", async (req, res) => {
    ...
    try {
        const reqtask: Task = req.body;
        const user = await userRepo.findOneBy({id: reqtask.creator.id});
        reqtask.creator = user as any as User;
